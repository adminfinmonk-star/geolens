...
(async () => {
  const j = JSON.parse(fs.readFileSync("design/stitch/gen_perception.json", "utf8"));
  if (j.error) {
    console.error(j.error);
    process.exit(1);
  }
  const inner = JSON.parse(j.result.content[0].text);
  fs.writeFileSync("design/stitch/gen_perception_parsed.json", JSON.stringify(inner, null, 2));
  const screen = inner.outputComponents[0].design.screens[0];
  fs.writeFileSync("design/stitch/perception-screen-id.txt", screen.id);
  console.log("id", screen.id, screen.title);
  const img = await get(screen.screenshot.downloadUrl);
  fs.writeFileSync("design/stitch/perception-screen.png", img.buf);
  console.log("png", img.status, img.buf.length);
  const html = await get(screen.htmlCode.downloadUrl);
  fs.writeFileSync("design/stitch/perception-screen.html", html.buf);
